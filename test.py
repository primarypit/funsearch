    ...
    def _draw_sample(self, content: str) -> str:
        ...
        while True:
            try:
                ...
                headers = {
                    'Authorization': 'Bearer sk-HI48B4tXuTc3H3UNLwue7UZdw1q32FrYyMEj0uEfOvCQsGGn',
...
class Sandbox_NS(evaluator.Sandbox):
    """Sandbox for executing generated code. Implemented by RZ.
...
if __name__ == '__main__':
    ...
    class_config_NS = config.ClassConfig(llm_class=LLMAPI, sandbox_class=Sandbox_NS)
    config = config.Config(samples_per_prompt=4)
    funsearch_programs = []

    profiler_F = profile.Profiler("../logs_FS/demo", island_num = config.programs_database.num_islands)
    profiler_NS = profile.Profiler("../logs_NS/demo", island_num = 1)

    for i in range(NS_time):
        if not os.path.exists('../program_NS_{}'.format(i)):
            os.makedirs('../program_NS_{}'.format(i))
        cur_novel_programs = funsearch_NS.Novelty_Step(
            specification=specification_NS,
            inputs=tsp_test_data_NS,
            config=config,
            max_sample_nums=6,
            class_config=class_config_NS,
            programs=funsearch_programs,
            profiler=profiler_NS,
            log_dir='../logs_NS/demo',
            program_dir='../program_NS_{}'.format(i)
        )

        print("cur_novel_programs length:", len(cur_novel_programs))
        if not os.path.exists('../program_FS_{}'.format(i)):
            os.makedirs('../program_FS_{}'.format(i))
        funsearch_programs = funsearch.FunSeach_Step(
            specification=specification,
            inputs=tsp_test_data_NS,
            config=config,
            max_sample_nums=100,
            class_config=class_config,
            programs=cur_novel_programs,
            profiler=profiler_F,
            log_dir='../logs_FS/demo',
            program_dir='../program_FS_{}'.format(i)
        )